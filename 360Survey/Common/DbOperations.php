...
function GetEmployeeDetails($con, $eid, &$employee_details)
{
  ...
  try {
    ...
    $employee_details_stmt = $con->prepare("SELECT Name, DepartmentID, JobTitleID, PhoneNumber, Email FROM Employee WHERE EID =?");
    ...
    if ($employee_details_row) {
      ...
      $employee_details['PhoneNumber'] = $employee_details_row['PhoneNumber'];
...
function GetEmployeeReportingManagers($con, $eid, &$rm_list)
{
  $rm_list = [];
  try {
    ...
    while ($row = $rm_stmt->fetch()) {
      ...
      if ($rm_details_row) {
        ...
        printf("Adding Reporting Manager EID:%s, Name:%s,JobTitleId:%s, EmailAddress:%s", $rm_eid, $rm_name,
          $rm_job_title_id, $rm_email);
        array_push($rm_list, [
          'EID' => $rm_eid,
          'Name' => $rm_name,
    ...
    echo "Error: GetEmployeeReportingManagers Failed for EmployeeId:" . $eid . ":" . $e->getMessage() . "\n";
  }
}
...
function GetEmployeeReportingManagersForSurveyAssignment($con, $eid, &$rm_list)
{
  ...
  try {
    ...
    while ($row = $rm_stmt->fetch()) {
      $rm_eid = $row['SEID'];
      echo "Looking for details of Reporting Manager EID=" . $rm_eid . "\n";
      $rm_details_stmt = $con->prepare("SELECT Name, Email FROM Employee WHERE EID =?");
      $rm_details_stmt->execute([$rm_eid]);
      $rm_details_row = $rm_details_stmt->fetch();
      if ($rm_details_row) {
        $rm_name = $rm_details_row['Name'];
        $rm_email = $rm_details_row['Email'];
        printf("Adding Reporting Manager EID:%s, Name:%s, EmailAddress:%s",
          $rm_eid, $rm_name, $rm_email);
        array_push($rm_list, [
          'EID' => $rm_eid,
          'Name' => $rm_name,
          'EmailAddress' => $rm_email
...
function GetEmployeeDirectReports($con, $eid, &$dr_list)
{
  $dr_list = [];
  try {
    ...
    echo "GetEmployeeDirectReports Failed for EmployeeId:" . $eid . ":" . $e->getMessage() . "\n";
  }
}
...
function GetEmployeeDirectReportsForSurveyAssignment($con, $eid, &$dr_list)
{
  ...
  try {
    ...
    while ($row = $dr_stmt->fetch()) {
      $dr_eid = $row['DirectReportEID'];
      echo "DirectReport EID=" . $dr_eid . "\n";
      $dr_details_stmt = $con->prepare("SELECT Name, Email FROM Employee WHERE EID =?");
      $dr_details_stmt->execute([$dr_eid]);
      $dr_details_row = $dr_details_stmt->fetch();
      if ($dr_details_row) {
        $dr_name = $dr_details_row['Name'];
...
function GetEmployeePeerList($con, $eid, &$peer_list)
{
  $peer_list = [];
  try {
    ...
    echo "GetEmployeePeerList Failed for EmployeeId:" . $eid . ":" . $e->getMessage() . "\n";
  }
}
...
function GetEmployeePeerListForSurveyAssignment($con, $eid, &$peer_list)
{
  ...
  try {
    ...
    while ($row = $peer_list_stmt->fetch()) {
      $peer_eid = $row['PeerID'];
      echo "Peer EID=" . $peer_eid . "\n";
      $peer_details_stmt = $con->prepare("SELECT Name, Email FROM Employee WHERE EID =?");
      $peer_details_stmt->execute([$peer_eid]);
      $peer_details_row = $peer_details_stmt->fetch();
      if ($peer_details_row) {
        $peer_name = $peer_details_row['Name'];
