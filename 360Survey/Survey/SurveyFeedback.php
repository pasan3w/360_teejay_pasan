...
function AddSurveyorFeedbackForEmployee($con, $survey_id, $surveyor_eid, $surveyor_type, $eid, &$feedback_array){
  ...
  try {
    ...
    $sql_rating = "INSERT INTO SurveyFeedback (SurveyID, EID, SurveyorEID, SurveyorType, QuestionCategoryID, ";
    $sql_rating .= "QuestionID, Rating) VALUES (?,?,?,?,?,?,?)";
    $insert_rating_feedback_stmt = $con->prepare($sql_rating);

    $sql_text = "INSERT INTO SurveyFeedbackText (SurveyID, EID, SurveyorEID, SurveyorType, QuestionCategoryID, ";
    $sql_text .= "QuestionID, Text) VALUES (?,?,?,?,?,?,?)";
    $insert_text_feedback_stmt = $con->prepare($sql_text);
    
    foreach ($feedback_array as $category_feedback_array) {
      printf("\t\tCategoryId=%d, QuestionNumber=%d, Type=%d, Rating=%d\n", $category_feedback_array[0], $category_feedback_array[1], $category_feedback_array[2], $category_feedback_array[3]);
      // if ($insert_feedback_stmt->execute([$category_feedback_array[0], $category_feedback_array[1], $category_feedback_array[2]])){
      if (0 == $category_feedback_array[2]){
        printf("Inserting Rating=%d to table SurveyFeedback\n", $category_feedback_array[3]);
        if ($insert_rating_feedback_stmt->execute([
          $survey_id,
          $eid,
          $surveyor_eid,
          $surveyor_type,
          $category_feedback_array[0],
          $category_feedback_array[1],
          $category_feedback_array[3]
        ])) {
         print("INSERT Feedback to table SurveyFeedback successful\n");
       } else {
          print("INSERT Feedback to table SurveyFeedback Failed\n");
       }
      }elseif (1 == $category_feedback_array[2]){
        printf("Inserting text=%s to table SurveyFeedbackText\n", $category_feedback_array[3]);
        if ($insert_text_feedback_stmt->execute([
          $survey_id,
          $eid,
          $surveyor_eid,
          $surveyor_type,
...
function AddSurveyorFeedbackForQuestion($con, $survey_id, $eid,
  $surveyor_eid, $surveyor_type, $category_id, $question_number, $type, $rating){
    global $SurveyorType2String;
    ...
    try {
      if (0 == $type){
        ...
        printf("Inserting text=%s to table SurveyFeedbackText\n", $rating);
        $sql = "INSERT INTO SurveyFeedbackText (SurveyID, EID, SurveyorEID, SurveyorType, QuestionCategoryID, ";
        $sql .= "QuestionID, Text) VALUES (?,?,?,?,?,?,?)";
        $insert_feedback_stmt = $con->prepare($sql);
        printf("Updating SurveyFeedbackText table: SurveyId=%d, EmployeeId=%s, SurveyorId=%s, SurveyorType=%s, CategoryId=%d, QuestionNumber=%d, Text=%s\n",
          $survey_id, $eid, $surveyor_eid, $SurveyorType2String[$surveyor_type], $category_id, $question_number, $rating);
        printf("SQL=[%s]\n", $sql);
        if ($insert_feedback_stmt->execute([$survey_id, $eid, $surveyor_eid, $surveyor_type, $category_id, $question_number, $rating])){
          print("INSERT Feedback successful\n");
...
function GetSurveyTextFeedbackForEmployeeBySurveyorType($con, $survey_id, $eid, &$feedback_array){
  global $SurveyorType2String;
  $feedback_array = [];
  echo "-------------------GetSurveyTextFeedbackForEmployeeBySurveyorType:[start]------------------------\n";
  $open_assignment_count = GetOpenAssignmnetCountForEmployeeInSurvey($con, $survey_id, $eid);
  printf("OpenAssignments for survey_id=%d for Employee=%s is %d", $survey_id, $eid, $open_assignment_count);
  if ($open_assignment_count){
    printf("Survey Results for Employee=%s not available, response pending from %d surveyors\n", $eid, $open_assignment_count);
  ...
  try{
    ...
    $survey_assignments_stmt->execute([$survey_id, $eid]);
    //$surveyor_map = [];
    static $feedback_sql = "SELECT QuestionCategoryID, QuestionID, Text";
    $feedback_sql .= " FROM SurveyFeedbackText WHERE SurveyID=? AND EID=? AND SurveyorEID=?";
    $feedbak_stmt = $con->prepare( $feedback_sql);
    while($row = $survey_assignments_stmt->fetch(PDO::FETCH_ASSOC)){
      //var_dump($row);
      $surveyor_eid = $row['SurveyorEID'];
      ...
      while ($feedback_row = $feedbak_stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        $question_category_id = $feedback_row['QuestionCategoryID'];
        $question_id = $feedback_row['QuestionID'];
        $feedback_txt = $feedback_row['Text'];
        $feedback_array[$surveyor_type][$surveyor_eid]['Feedback'][$question_category_id][$question_id]=$feedback_txt;
        printf("\tQuestionCategoryID=%d, QuestionID=%d, Text=%s\n",
          $question_category_id, $question_id, $feedback_txt);
      }
    }
  }catch(PDOException $e) {
    echo "GetSurveyTextFeedbackForEmployeeBySurveyorType: Failed due to DB Exception: " . $e->getMessage() . "\n";
  }
}
