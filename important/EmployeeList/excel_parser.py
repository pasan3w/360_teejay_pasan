...
def LoadEmployeeData(excel_file_path, app_config, employee_dic):
  ...
  for row in range(int(app_config['company.employee.excelconfig']['StartRow']), ws.max_row + 1):
    srow = str(row)
    employee_id = ws[employee_id_col + srow].value
    if (not employee_id):
...
def PrintManagersDic(managers_dic):
  print("************PrintManagersDic[start]************")        
  for key in managers_dic.keys():
    reporting_managers = []
    direct_reports = []
    peer_list = []
    print("--------ManagerEID={}, Name={}--------".format(key, managers_dic[key]['Name']))
    job_roles = managers_dic[key]["job_roles"].keys()
    for job_role in job_roles:
      print("JobRole=", job_role)
      print("\tReportingManagers=", managers_dic[key]["job_roles"][job_role]["ReportingManagers"])
      if managers_dic[key]["job_roles"][job_role]["ReportingManagers"]:
        for rm_eid in str(managers_dic[key]["job_roles"][job_role]["ReportingManagers"]).split(','):
          rm_eid = rm_eid.strip()
          if rm_eid not in reporting_managers:
            reporting_managers.append(rm_eid)
      print("\tDirectReports=", managers_dic[key]["job_roles"][job_role]["DirectReports"])
      if managers_dic[key]["job_roles"][job_role]["ReportingManagers"]:
        for dr_eid in str(managers_dic[key]["job_roles"][job_role]["DirectReports"]).split(','):
          dr_eid = dr_eid.strip()
          if dr_eid not in direct_reports:
            direct_reports.append(dr_eid)
      print("\tPeerList=", managers_dic[key]["job_roles"][job_role]["PeerList"])
      if managers_dic[key]["job_roles"][job_role]["PeerList"]:
        for pr_eid in str(managers_dic[key]["job_roles"][job_role]["PeerList"]).split(','):
          pr_eid = pr_eid.strip()
          if pr_eid not in peer_list:
            peer_list.append(pr_eid)
    print("Integrated ReportingManagers=", reporting_managers)
    print("Integrated ReportingManagers=[{}]".format(','.join(reporting_managers)))
    print("Integrated DirectReports=",direct_reports)
    print("Integrated DirectReports=[{}]".format(','.join(direct_reports)))
    print("Integrated PeerList=[{}]".format(','.join(peer_list)))

  print("************PrintManagersDic[end]************")        
